                    ...
                    else
                    {
                        ...
                        try
                        {
                            using (archive = ZipFile.Open(tempPath, ZipArchiveMode.Update))
                            {
                                ...
                                if (System.IO.File.Exists(Path.Combine(Path.GetTempPath(), "temp.zip")))
                                    System.IO.File.Delete(Path.Combine(Path.GetTempPath(), "temp.zip"));
                            }
                        }
                        catch (Exception ex) { throw ex; }
                    }
                            ...
                            if (names.Length == 1)
                            {
                                ...
                                while (reader.Read())
                                {
                                    detailFiles = new FileDetails
                                    {
                                        ...
                                        Modified = reader["DateModified"].ToString(),
                                        Created = reader["DateCreated"].ToString()
                                    };
                ...
                using (SqlCommand myCommand = new SqlCommand(query, sqlConnection))
                {
                    ...
                    using (SqlDataReader myReader = myCommand.ExecuteReader())
                    {
                        while (myReader.Read())
                        {
                            try
                            {
                                byte[] imageBinary = (byte[])myReader["Content"];
            ...
            try
            {
                ...
                foreach (IFormFile file in uploadFiles)
                {
                    if (uploadFiles != null)
                    {
                        ...
                        {
                            FileManagerResponse detailsResponse = GetFiles(path, false, data);
